...
Use **Save draft** at any time to persist only the decisions already completed. Blank rows remain pending, previously saved decisions are preserved, and the workflow does not advance. **Validate and continue** is the final action: it requires every current value to be resolved or explicitly ignored before enabling the migration-plan preview.
