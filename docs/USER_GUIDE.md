...
Choose a **Default ticket entity** when creating or editing a migration profile. It defines both the import scope and the final fallback. The plan first honors an explicitly mapped entity, then uses a resolved requester only when that user has exactly one usable GLPI entity inside the project scope. With no usable entity or several entities, it uses the explicit migration-profile location/entity association for the resolved location. If neither rule succeeds, it uses the migration project's default entity. The requester's preferred entity, GLPI's root entity, the location's native scope and hierarchy-name similarity never silently override this order.
...
When locations are mapped, the same screen displays **Location and entity associations**. GLPI stores an `entities_id` scope on each location, normally inherited from the active entity when the location is created. GLPI 11 does not expose this value as a standard editable field on the Location form, so it is informational only. Select an explicit association scoped to this migration profile. This is the authoritative safeguard for tickets whose requester has no unique usable entity: one unique requester entity remains prioritary, then the explicit location association, then the project default.
